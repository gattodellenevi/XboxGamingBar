    ...
    internal class ProfileManager : Manager
    {
        ...
        public ProfileManager(AppServiceConnection connection) : base(connection)
        {
            ...
            if (!File.Exists(globalProfilePath))
            {
                ...
                GlobalProfile = new GameProfile(GameProfile.GLOBAL_PROFILE_NAME, GameProfile.GLOBAL_PROFILE_NAME, true, 25, true, 80, 0, 0, 0, globalProfilePath, gameProfiles);
                GlobalProfile.Save();
            }
            else
            {
        ...
        public GameProfile AddNewProfile(GameId gameId)
        {
            ...
            var newGameProfile = new GameProfile(gameId.Name, gameId.Path, true, CurrentProfile.TDP, CurrentProfile.CPUBoost, CurrentProfile.CPUEPP, CurrentProfile.CPUClock, CurrentProfile.FPSLimit, CurrentProfile.FPSLimitMode, newGameProfilePath, gameProfiles);
            newGameProfile.Save();
